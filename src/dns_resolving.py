...
class DomainName(str):
   ...
   def __init__(self, *args, **kwargs):
      ...
      if (len(self) > self.NAME_LENGTH_LIMIT):
         raise ValueError('%r is longer than %r, which is the maximum length allowed for domain names' % 
            (self, self.NAME_LENGTH_LIMIT))
            ...
            if (l > cls.LABEL_LENGTH_LIMIT):
               if (l >= 192):
                  ...
                  if (comp_depth > cls.COMP_DEPTH_LIMIT):
                     ...
                     raise ValueError('Compression depth limit %d exceeded.' % (cls.COMP_DEPTH_LIMIT))
                  if (stream_pos is None):
                     stream_pos = binstream.tell()
                  binstream.seek(offset)
                  continue
               raise ValueError('Label length %d in stream is greater than allowed maximum %d.' % (l, cls.LABEL_LENGTH_LIMIT))
         
            label = binstream.read(l)
            if (len(label) != l):
               raise ValueError('Insufficient data in stream for label of length %d.' % (l,))
   ...
   def __binary_repr_compute(self):
      ...
      if ('' in elements):
         raise ValueError('Empty label in %r' % (self,))
      
      elements.append('')
      
      for element in elements:
         if not (len(element) <= self.LABEL_LENGTH_LIMIT):
            raise ValueError('Element %r of name %r is longer than 63 octets' % (element,name))
      
      rv = ''.join([(struct.pack('>B', len(e)) + e) for e in elements])
      return rv
...
class DNSReprBase(object):
   def __repr__(self):
      return '%s(%s)' % (self.__class__.__name__, ', '.join(['%s=%r' % (name,getattr(self,name)) for name in self.fields]))
...
# ----------------------------------------------------------------------------- RDATA sections
class RDATA_Meta(type):
   """RDATA Classes collector"""
   def __init__(self, *args, **kwargs):
      type.__init__(self, *args, **kwargs)
      if (hasattr(self, 'type')):
         RDATA.rdata_type_register(self)


class RDATA(DNSReprBase):
   __metaclass__ = RDATA_Meta
   RDATA_TYPES = {}
   fields = ('rdata',)
   def __init__(self, *args, **kwargs):
      ...
      if (len(fields) != len(args)):
         raise TypeError('Got %d non-keyword arguments, expected %d (given %d keyword arguments).' %
            (len(args), len(fields), len(kwargs)))
...
class RDATA_A(RDATA):
   ...
   def build_from_binstream(cls, binstream, rdlength):
      ...
      if (len(rdata) != 4):
         raise ValueError('Rdata %r has invalid length; expected 4 octets.' % (rdata,))
...
class RDATA_DomainName(RDATA):
   ...
   def build_from_binstream(cls, binstream, rdlength):
      ...
      if not (pos_end == binstream.tell()):
         raise ValueError('Rdata %r is not a valid domain name.' % (rdata,))
...
class RDATA_SOA(RDATA):
   ...
   def build_from_binstream(cls, binstream, rdlength):
      ...
      if (len(srrem_str) != 20):
         raise ValueError('Rdata %r is not a valid SOA record' % (rdata,))
...
class RDATA_MX(RDATA):
   ...
   def build_from_binstream(cls, binstream, rdlength):
      ...
      if (len(pref_str) < 2):
         raise ValueError('Rdata %r is not a valid MX record.' % (rdata,))
...
class RDATA_TXT(RDATA):
   ...
   def __init__(self, txt_data):
      ...
      for txt in txt_data:
         if (len(txt) > 255):
            raise ValueError('TXT fragment %r is too long (maximum is 255 bytes).')

   def binary_repr(self):
      """Return binary representation of this RDATA in DNS protocol"""
      for txt in self.txt_data:
         if (len(txt) > 255):
            raise ValueError('Txt fragment %r is too long.' % (txt,))
      return ''.join(['%s%s' % (struct.pack('>B', len(txt)), txt) for txt in
         self.txt_data])
   ...
   def build_from_binstream(self, binstream, rdlength):
      ...
      txt_data = []
      while (i < rdlength):
         (slen,) = struct.unpack('>B', rdata[i])
         j = i + 1 + slen
         txt = rdata[i+1:j]
         if (len(txt) < slen):
            raise ValueError('Rdata %r is not a valid TXT record' % (rdata,))
         txt_data.append(txt)
         i = j
...
class RDATA_AAAA(RDATA):
   ...
   def build_from_binstream(cls, binstream, rdlength):
      if (rdlength != 16):
         raise ValueError('Length of AAAA record must be 16 octets; got %r.' % (rdlength,))
      rdata = cls.rdata_read(binstream, rdlength)
...
class ValueVerifier:
   ...
   def name_validate(cls, name):
      if not (cls.NAME_MIN <= len(name) <= cls.NAME_MAX):
         raise ValueError('NAME %r is invalid' % (name,))
   
   @classmethod
   def type_validate(cls, rtype):
      if not (cls.TYPE_MIN <= int(rtype) <= cls.TYPE_MAX):
         raise ValueError('TYPE %r is invalid' % (rtype,))
...
class DNSQuestion(ValueVerifier, DNSReprBase):
   ...
   def build_from_binstream(cls, binstream):
      ...
      (rtype, rclass) = struct.unpack('>HH', tc_str)
      return cls(name, rtype, rclass)
   
   def binary_repr(self):
      """Return binary representation of this question section"""
      return (self.name.binary_repr() + struct.pack('>HH', self.type, 
         self.rclass))
...
class ResourceRecord(ValueVerifier, DNSReprBase):
   ...
   def build_from_binstream(cls, binstream):
      ...
      (rtype, rclass, ttl, rdlength) = struct.unpack('>HHLH', tctl_str)
   ...
   def binary_repr(self):
      ...
      return (self.name.binary_repr() + struct.pack('>HHLH', self.type,
...
class DNSHeader(DNSReprBase):
   ...
   def limit_verify(limit_min, limit_max, val):
      if not (limit_min <= val <= limit_max):
         raise ValueError('Expected value to lie between %r and %r; got %r instead.' % (limit_min, limit_max, val))
   ...
   def build_from_binstring(cls, binstring):
      if (len(binstring) != 12):
         raise ValueError('Binstring %r has invalid length' % (binstring,))
      
      (id, flags_1, flags_2, qdcount, ancount, nscount, arcount) = \
         struct.unpack('>HBBHHHH', binstring)
   ...
   def binary_repr(self):
      ...
      return struct.pack('>HBBHHHH', self.id, flags_1, flags_2, self.qdcount,
...
class DNSFrame(DNSReprBase):
   ...
   def binary_repr(self):
      ...
      return (self.header.binary_repr() +
         ''.join([s.binary_repr() for s in (tuple(self.questions) +
...
class DNSQuery:
   ...
   def __init__(self, lookup_manager, result_handler, id, question, timeout):
      if (not callable(question.binary_repr)):
         raise ValueError('Value %r for argument question is invalid' % (question,))
      ...
      if not (timeout is None):
         self.tt = self.la.event_dispatcher.Timer(timeout, self.timeout_process, parent=self)
...
class DNSLookupManager:
   ...
   def __init__(self, event_dispatcher, ns_addr, addr_family=socket.AF_INET):
      ...
      self.sock = self.event_dispatcher.SockDatagram(
         input_handler=self.data_process, close_handler=self.close_process)
      self.sock.connection_init(addr_family, socket.SOCK_DGRAM)
      self.cleaning_up = False
      if (not (len(ns_addr) == 2)):
         raise ValueError('Argument ns_addr should have two elements; got %r' % (ns_addr,))
      
      # Normalize ns_addr argument
      self.ns_addr = (str(ip_address.ip_address_build(ns_addr[0])), int(ns_addr[1]))
      self.queries = {}
   
   def data_process(self, source, data):
      try:
         dns_frame = DNSFrame.build_from_binstream(cStringIO.StringIO(data))
      except ValueError:
         self.log(30, '%r got udp frame %r not parsable as dns data from %r. Ignoring. Parsing error was:' % (self, data, source), exc_info=True)
         return
      
      if (source != self.ns_addr):
         self.log(30, '%r got spurious udp frame from %r; target NS is at %r. Ignoring.' % (self, source, self.ns_addr))
         return
      
      if (not (dns_frame.header.id in self.queries)):
         self.log(30, '%r got spurious (unexpected id) query dns response %r from %r. Ignoring.' % (self, dns_frame, source))
      ...
      for query in self.queries[dns_frame.header.id][:]:
         ...
         self.log(30, '%r got spurious (unexpected question section) query dns response %r from %r. Ignoring.' % (self, dns_frame, source))
   ...
   def query_add(self, query):
      ...
      if (query in query_list):
         raise ValueError('query %r is already registered with %r.' % (query, self))
      
      query_list.append(query)
      
      self.sock.send_data(dns_frame_str, self.ns_addr)
   ...
   def close_process(self, fd):
      ...
      if (not self.cleaning_up):
         self.log(30, 'UDP socket of %r is unexpectedly being closed.' % (self,))
         self.sock = None
      ...
      for result in results:
         ...
         for answer in result.answers:
            if (not (answer.name in names_valid)):
               self.log(30, "%r got bogus answer %r; didn't expect this name. Ignoring." % (self, answer)) 
               continue
            if (answer.type == RDATA_CNAME.type):
               ...
               self.log(30, "%r got bogus answer %r; didn't expect this type. Ignoring." % (self, answer))
               continue
...
def _module_selftest_local():
   question_1 = DNSQuestion(DomainName('www.example.net'),RDATA_A.type,1)
   question_2 = DNSQuestion(DomainName('example.net'),RDATA_AAAA.type,1)
   
   f1_0 = DNSFrame(questions=[question_1, question_2],id=1234)
   r1_0 = f1_0.binary_repr()
   f1_1 = DNSFrame.build_from_binstream(cStringIO.StringIO(r1_0))
   r1_1 = f1_1.binary_repr()
   if (r1_0 != r1_1):
      raise StandardError('Idempotency test 01 failed.')
   
   answer_1 = ResourceRecord(name=DomainName('www.example.net'), rtype=RDATA_AAAA.type, 
      rdata=RDATA_AAAA(ip_address.ip_address_build('::1')), ttl=0)
   answer_2 = ResourceRecord(name=DomainName('www.example.net'), rtype=RDATA_A.type, 
      rdata=RDATA_A(ip_address.ip_address_build('127.0.0.1')), ttl=0)
   ...
   f2_1 = DNSFrame.build_from_binstream(cStringIO.StringIO(r2_0))
...
def _module_selftest_network(ns_addr, af=socket.AF_INET):
   ...
   query_data = DNSFrame(questions=[DNSQuestion(DomainName('www.example.net'), QTYPE_ALL)], id=1236).binary_repr()
   
   s.sendto(query_data, ns_addr)
   print('Sending query %r to %r ...' % (query_data, ns_addr,))
   (in_data, in_addr) = s.recvfrom(512)
   print('Got reply %r from %r.' % (in_data, in_addr))
   
   response_data = DNSFrame.build_from_binstream(cStringIO.StringIO(in_data))
   print('Data: %r' % response_data)
   return True
...
class _StatefulLookupTester_2:
   ...
   def result_handler(self, dns_query, dns_data):
      print('Got response: %r' % (dns_data,))
      print('Ipv4/IPv6 addresses: %r' % (dns_data.get_rr_ip_addresses(),))
      self.ed.shutdown()


def _module_selftest_stateful_network(ns_addr):
   import fd_management
   q1 = _StatefulLookupTester_1(fd_management.EventDispatcherSelect(), ns_addr, DNSQuestion(DomainName('www.example.net'),QTYPE_ALL))
   q2 = _StatefulLookupTester_2(fd_management.EventDispatcherSelect(), ns_addr, 'sixxs.net', qtypes=(RDATA_A.type, RDATA_AAAA.type))
...
if (__name__ == '__main__'):
   ...
   handler_stderr.setLevel(0)
   handler_stderr.setFormatter(formatter)
   logger.addHandler(handler_stderr)
   logging.getLogger('gonium.fd_management').setLevel(21)
